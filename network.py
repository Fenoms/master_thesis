...
def comparisonNet_generator(block_fn, data_format = None):
    ...
    def model(inputs, is_training):
        ...
        inputs = block_layer(inputs = inputs, filters = 128, block_fn = 'bottleneck_block',
                                    blocks = 1, strides = 2, is_training = is_training, name = 'compare_1', data_format = data_format)


        inputs = block_layer(inputs = inputs, filters = 64, block_fn = 'bottleneck_block',
                                    blocks = 1, strides = 2, is_training = is_training, name = 'compare_2', data_format = data_format) 

        inputs = tf.layers.avergae_pooling2d(inputs = inputs, pool_size = 2, strides = 2, data_format = data_format, name = 'final_pool')
...
class FewshotsNet:
    ...
    def __init__(self, support_set_images, support_set_labels, query_images, query_labels, data_format,
        ...
        self.e = embeddingResNet_generator(block_fn = 'building_block', layers = [2,2,2,2], data_format = 'channels_last')
        self.c = comparisonNet_generator(block_fn = 'bottleneck_block', data_format = data_format)
    ...
    def loss(self):
        ...
        with tf.name_scope('losses'):
            ...
            for image in tf.unstack(self.query_images, axis = 1):
                image_embedding = self.e(inputs = image, is_training = self.is_training)
            ...
            inputs = tf.reshape(inputs, shape = [batch_size* num_classes*num_samples_per_class*query_size, -1, -1, -1])
            #shape of inputs: [batch_size*num_classes* num_samples_per_class*query_size, d, d, 2*c]

            scores = self.c(block_fn = 'bottleneck_block', data_format = data_format)


            scores = tf.reshape(output, shape = [self.batch_size, self.num_classes * self.num_samples_per_class, query_size])

            support_set_labels = tf.expand_dims(support_set_labels, axis = 2)
            ...
            msq_losses = tf.reduce_sum(tf.pow(labels, scores)) / self.batch_size
            msq_losses = tf.identity(msq_losses, 'loss')
            tf.summary.scalar('loss', msq_losses)

            prediction = tf.argmax(tf.reduce_mean(scores, axis = 2), axis = 1)
            correct_prediction = tf.equal(prediction, tf.cast(self.query_labels, tf.int64))
            accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))/ self.batch_size
    ...
    def train(self, losses):
        ...
        with tf.control_dependencies(update_ops):
            ...
            ada_opt = opt.minimize(losses['loss'], var_list  = train_variables)
